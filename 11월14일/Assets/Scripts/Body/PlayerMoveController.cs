...
public class PlayerMoveController : MonoBehaviour
{
    ...
    //|| (!targetHit.collider.CompareTag("Cube") && !targetHit.collider.CompareTag("CubeBorder")) &&
    void ClimbPlayer()
    {
        ...
        if (Physics.Raycast(forwardRay, out forwarhit, 0.4f) && forwarhit.collider.CompareTag("Cube"))
        {
            ...
            //Vector3 rayStartPoint = transform.position + new Vector3(0.3f, 0, 0);
        ...
        if (Physics.Raycast(playerBodyRay_F, out playerbodyRayHit_F, 0.8f) && playerbodyRayHit_F.collider.CompareTag("Cube"))
        {
            ...
            if (Physics.Raycast(f_Ray, out f_Hit, 0.8f) && f_Hit.collider.CompareTag("player")) // 플레이어가 큐브 forward방향에 매달린 경우
            {
                if (Input.GetKeyDown(KeyCode.A) && isHoldMode && !isHangingMoving &&
                    Physics.Raycast(r_Ray, out r_Hit, 0.8f) && r_Hit.collider.CompareTag("Cube") &&
                    !Physics.Raycast(playerBodyRay_L, out playerbodyRayHit_L, 1.1f) && playerbodyRayHit_L.collider == null)
                {
                ...
                if (Input.GetKeyDown(KeyCode.D) && isHoldMode && !isHangingMoving &&
                    Physics.Raycast(l_Ray, out l_Hit, 0.8f) && l_Hit.collider.CompareTag("Cube") &&
                    !Physics.Raycast(playerBodyRay_R, out playerbodyRayHit_R, 1.1f) && playerbodyRayHit_R.collider == null)
                {
            ...
            {
                if (Input.GetKeyDown(KeyCode.A) && isHoldMode && !isHangingMoving &&
                    Physics.Raycast(b_Ray, out b_Hit, 0.8f) && b_Hit.collider.CompareTag("Cube") &&
                    !Physics.Raycast(playerBodyRay_L, out playerbodyRayHit_L, 0.8f) && playerbodyRayHit_L.collider == null)
                {
                ...
                if (Input.GetKeyDown(KeyCode.D) && isHoldMode && !isHangingMoving &&
                    Physics.Raycast(f_Ray, out f_Hit, 0.8f) && f_Hit.collider.CompareTag("Cube") &&
                    !Physics.Raycast(playerBodyRay_R, out playerbodyRayHit_R, 0.8f) && playerbodyRayHit_R.collider == null)
                {
