...
public class BlockController : MonoBehaviour  // 블록 낙하 관련 코드
{
    ...
    private Transform[] blockBorderTransforms;  // BlockBorder Transform 저장하는 배열
    // Start is called before the first frame update
    void Start()
    {
        blockTransform = transform;  // 현재 블록의 Transform 컴포넌트를 초기화
        blockBorderTransforms = new Transform[4];  // 블록당 4개의 BlockBorder 변환을 가정

        // 만약 BlockBorder 오브젝트의 이름이 "BlockBorder1", "BlockBorder2", 등으로 되어 있다면
        for (int i = 0; i < 4; i++)
        {
            blockBorderTransforms[i] = transform.Find("BlockBorder" + (i + 1));
        }
    }
