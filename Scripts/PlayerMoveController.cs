...
public class PlayerMoveController : MonoBehaviour {
  ...
  public float maxMoveRange =
      4.0f; // Maximum allowed movement range for the player.
  ...
  private Quaternion targetRotation =
      Quaternion.identity; // The target rotation for the player.
  ...
  void PlayerMovement() {
    ...
    float horizontalInput =
        Input.GetAxis("Horizontal"); // Horizontal movement input (-1 to 1).
    float verticalInput =
        Input.GetAxis("Vertical"); // Vertical movement input (-1 to 1).
  ...
  void PlayerRotation() {
    Quaternion currentRotation =
        transform.rotation; // Current rotation of the player.
    ...
    if (isRotating) {
      ...
      if (Input.GetKeyDown(KeyCode.E)) {
        ...
        isRotating = true; // Set the rotation flag to true to indicate rotation
                           // is in progress.
