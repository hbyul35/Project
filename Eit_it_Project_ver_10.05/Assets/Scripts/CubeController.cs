  ...
  void Update() {
    ...
    if (Physics.Raycast(ray, out hit, raycastDistance)) {
      ...
      if (hit.collider.CompareTag("Cube")) {
        ...
        isRotating =
            transform.parent.GetComponent<PlayerMoveController>()
                .isRotating; // PlayerMoveController::isRotating 회전 안할때
        holdMode = transform.parent.GetComponent<PlayerMoveController>()
                       .isHoldMode; // PlayerMoveController::holdmode 껏을때

        if (Input.GetKey(KeyCode.W) && Input.GetKeyDown(KeyCode.Space) &&
            !isMoving && !isRotating && !holdMode) {
