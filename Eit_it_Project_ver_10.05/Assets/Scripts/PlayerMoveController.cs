...
public class PlayerMoveController : MonoBehaviour {
    ...
    private HeadCollisionCheck
    headCollisionCheck; // HeadCollisionCheck 정보를 담을 변수
    ...
    void ClimbPlayer() {
        ...
        if (Physics.Raycast(forwardRay, out forwarhit, 0.3f) &&
                forwarhit.collider.CompareTag("Cube") &&
            ...
            headCollisionCheck =
                GetComponentInChildren<HeadCollisionCheck>(); // 머리 충돌체크용
            bool isHeadColliding = headCollisionCheck.isColliding;

            Ray targetCubeRay =
                new Ray(targetCube.transform.position,
                        targetCube.transform.up); // 타겟 큐브 위쪽 방량으로 레이생성
            RaycastHit targetHit;

            // 1. 올라가려는 큐브 위에 연속된 큐브가 있으면 막음 2. 공중, 떨어지는
            // 도중 등반하기 막음 3. 플레이어의 머리위에 큐브가 있을때 막음
            if (!Physics.Raycast(targetCubeRay, out targetHit, 1.0f) &&
                    Physics.Raycast(downwardRay, out downwarhit, 0.6f) && !isMoving &&
                    !isHeadColliding) {
    ...
    void HangingMode() // 매달려서 이동 10/05 좌우 방향만 구현
    {
        ...
        if (Input.GetKey(KeyCode.G) && Input.GetKeyDown(KeyCode.S) &&
                Physics.Raycast(upwardRay, out upwarhit, 0.5f) &&
                upwarhit.collider.CompareTag("Cube") && !isHoldMode) {
            targetCube = upwarhit.collider.gameObject;
            StartCoroutine(F_HangOnCube(targetCube));
        } else if (Input.GetKey(KeyCode.G) && Input.GetKeyDown(KeyCode.W) &&
                   Physics.Raycast(upwardRay, out upwarhit, 0.5f) &&
                   upwarhit.collider.CompareTag("Cube") && !isHoldMode) {
            targetCube = upwarhit.collider.gameObject;
        ...
        if (Physics.Raycast(forwardRay, out forwarhit, 0.4f) &&
                forwarhit.collider.CompareTag("Cube")) {
            targetCube = forwarhit.collider.gameObject;

            Ray f_Ray =
                new Ray(targetCube.transform.position, targetCube.transform.forward);
            Debug.DrawRay(targetCube.transform.position,
                          targetCube.transform.forward * 1.5f, Color.red); // 앞

            Ray b_Ray =
                new Ray(targetCube.transform.position, -targetCube.transform.forward);
            Debug.DrawRay(targetCube.transform.position,
                          -targetCube.transform.forward * 1.5f, Color.blue); // 뒤

            Ray r_Ray =
                new Ray(targetCube.transform.position, targetCube.transform.right);
            Debug.DrawRay(targetCube.transform.position,
                          targetCube.transform.right * 1.5f, Color.yellow); // 오른쪽

            Ray l_Ray =
                new Ray(targetCube.transform.position, -targetCube.transform.right);
            Debug.DrawRay(targetCube.transform.position,
                          -targetCube.transform.right * 1.5f, Color.magenta); // 왼쪽

            Ray u_Ray =
                new Ray(targetCube.transform.position, targetCube.transform.up);
            Debug.DrawRay(targetCube.transform.position,
                          targetCube.transform.up * 1.5f, Color.white); // 위쪽

            Ray d_Ray =
                new Ray(targetCube.transform.position, -targetCube.transform.up);
            Debug.DrawRay(targetCube.transform.position,
                          -targetCube.transform.up * 1.5f, Color.black); // 아래쪽
            ...
            //---------Left/Right Movement---------
            // forward
            if (Physics.Raycast(f_Ray, out f_Hit, 0.7f) &&
                    f_Hit.collider.CompareTag(
                        "player")) // 큐브 앞쪽에 플레이어가 충돌하면
            {
                if (Input.GetKeyDown(KeyCode.A) && isHoldMode && !isHoldMoving &&
                        Physics.Raycast(r_Ray, out r_Hit, 0.5f) &&
                        r_Hit.collider.CompareTag("Cube")) {
                    targetCube = r_Hit.collider.gameObject;
                    StartCoroutine(MoveLeftHangingOnCube());
                } else if (Input.GetKeyDown(KeyCode.D) && isHoldMode && !isHoldMoving &&
                           Physics.Raycast(l_Ray, out l_Hit, 0.5f) &&
                           l_Hit.collider.CompareTag("Cube")) {
                    targetCube = l_Hit.collider.gameObject;
                    StartCoroutine(MoveRightHangingOnCube());
                }
            }
            // back
            if (Physics.Raycast(b_Ray, out b_Hit, 0.7f) &&
                    b_Hit.collider.CompareTag("player")) {
                if (Input.GetKeyDown(KeyCode.A) && isHoldMode && !isHoldMoving &&
                        Physics.Raycast(l_Ray, out l_Hit, 0.5f) &&
                        l_Hit.collider.CompareTag("Cube")) {
                    targetCube = l_Hit.collider.gameObject;
            ...
            // right
            if (Physics.Raycast(r_Ray, out r_Hit, 0.7f) &&
                    r_Hit.collider.CompareTag("player")) {
                if (Input.GetKeyDown(KeyCode.A) && isHoldMode && !isHoldMoving &&
                        Physics.Raycast(b_Ray, out b_Hit, 0.5f) &&
                        b_Hit.collider.CompareTag("Cube")) {
                    targetCube = b_Hit.collider.gameObject;
                    StartCoroutine(MoveLeftHangingOnCube());
                } else if (Input.GetKeyDown(KeyCode.D) && isHoldMode && !isHoldMoving &&
                           Physics.Raycast(f_Ray, out f_Hit, 0.5f) &&
                           f_Hit.collider.CompareTag("Cube")) {
                    targetCube = f_Hit.collider.gameObject;
                    StartCoroutine(MoveRightHangingOnCube());
                }
            }
            // left
            if (Physics.Raycast(l_Ray, out l_Hit, 0.7f) &&
                    l_Hit.collider.CompareTag("player")) {
                if (Input.GetKeyDown(KeyCode.A) && isHoldMode && !isHoldMoving &&
                        Physics.Raycast(f_Ray, out f_Hit, 0.5f) &&
                        f_Hit.collider.CompareTag("Cube")) {
                    targetCube = f_Hit.collider.gameObject;
            ...
            //---------Up/Down Movement---------
            if (Physics.Raycast(f_Ray, out f_Hit, 0.7f) &&
                    f_Hit.collider.CompareTag("player") ||
                    Physics.Raycast(b_Ray, out b_Hit, 0.7f) &&
                    b_Hit.collider.CompareTag("player") ||
                    Physics.Raycast(r_Ray, out r_Hit, 0.7f) &&
                    r_Hit.collider.CompareTag("player") ||
                    Physics.Raycast(l_Ray, out l_Hit, 0.7f) &&
                    l_Hit.collider.CompareTag("player")) {
                if (Input.GetKeyDown(KeyCode.S) && isHoldMode && !isHoldMoving &&
                        Physics.Raycast(d_Ray, out d_Hit, 0.5f) &&
                        d_Hit.collider.CompareTag("Cube")) {
    ...
    IEnumerator F_HangOnCube(
        GameObject targetCube) // 매달리기 동작이 완성되면 종료됨 < 기억해
    {
        ...
        while (Time.time - startTime <
                journeyDuration) // 현재까지의 시간 < 이동에 필요한 전체 시간
        {
            float distanceCovered =
                (Time.time - startTime) * moveSpeed; // 현재까지 이동한 거리
            float fractionOfJourney = distanceCovered / journeyLength;
    ...
    IEnumerator B_HangOnCube(GameObject targetCube) {
        ...
        transform.rotation = Quaternion.LookRotation(
                                 targetCube.transform.position - transform.position); // 최종 회전 설정
    }
